...
/*
 * Dieser Klasse hat eine Methode der die Serie übergeben wird. Sie durchsucht die Websites nach
 * meinem Fav streamer. Die Websites klassen geben den Most Fav link zurück. Diese Links werden hier
 * verglichen. Und der Beste wird dann hier geöffnet.
 * 
 * @author Aljoscha
 *
 */
...
public class MainWebAccessController {
  ...
  public boolean startSerie() {
    ...
    try {
      ...
      if (StreamingSettings.LanguageTag.equals("de")) { // XXX Language Setting
        try {
      ...
      switch (e1.getTyp()) {
        case FAVORITENOTFOUND:
          Platform.runLater(() -> {
            OptionPanes.showConfirm("Keine Hoster",
                ...
                "Passe deine favorisierten Hoster an oder wähle manuell einen Hoster über 'Manuell Starten' aus.");
          });
          break;
          ...
          if (!this.tryed) {
            ...
            Platform.runLater(() -> {
              OptionPanes.showConfirm("Nicht gefunden",
                  "Ich konnte leider keine weitere Staffel finden...",
        ...
        case SEASONNOTFOUND:
          Platform.runLater(() -> {
            OptionPanes.showConfirm("Nicht gefunden",
                "Ich konnte leider keine weiteren Folgen finden...",
                "Viellicht gibt es bald neue!");
          });
          break;
        case UNKNOWN:
          Platform.runLater(() -> {
            OptionPanes.showConfirm("Nicht gefunden", "Es scheint ein Fehler aufgetreten zu sein.",
                "Beachte, dass aus technischen Gründen nur die Seite bs.to für englisch sprachige serien zur verfügung steht. Falls deine Serie keinen bs.to link hat wird dies also nicht funktionieren. Füge einen link manuell hinzu oder schaue einfach auf deutsch weiter.");
          });
          break;
        default:
          break;
  ...
  public static Links searchLinks(String name) {
    ...
    /*
     * TODO Deactivated until later if ((url = Movie4k.searchShow(name)) != null) { try {
     * l.setMovie4k(new URL(url)); } catch (MalformedURLException e) { // TODO Auto-generated catch
     * block e.printStackTrace(); } }
     */

    /*
     * TODO Deactivated until later if ((url = SerienStream.searchShow(name)) != null) { try {
     * l.setSerienstream(new URL(url)); } catch (MalformedURLException e) { // TODO Auto-generated
     * catch block e.printStackTrace(); } }
     */

    if (l.getBs() == null && l.getDeddl() == null && l.getMovie4k() == null
        && l.getSerienstream() == null) {
      return null;
    }
